...
export default function Sidebar() {
  ...
  const allMenuItems = [
    { name: "Dashboard", href: "/admin", icon: "M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6", roles: ["SUPER_ADMIN"] },
    ...
    { name: "Persetujuan Penjual", href: "/admin/seller-approvals", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z", roles: ["SUPER_ADMIN"] },
    { name: "Manajemen Staf", href: "/admin/staff", icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z", roles: ["SUPER_ADMIN"] },
    { name: "Kategori", href: "/admin/categories", icon: "M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z", roles: ["SUPER_ADMIN"] },
  ...
  return (
    <aside className="w-[260px] bg-white min-h-screen flex flex-col hidden md:flex border-r border-gray-200 shadow-[2px_0_8px_rgba(0,0,0,0.02)]">
      {/* Logo Area */}
      <div className="h-16 flex items-center px-6 border-b border-gray-200 shrink-0">
        <Link href="/admin" className="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto py-6">
        <div className="px-4 mb-2">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Menu Utama</p>
        </div>
        <nav className="space-y-1">
          {menuItems.map((item) => {
            // Find the best match by sorting href length descending
            const bestMatch = [...menuItems]
              .sort((a, b) => b.href.length - a.href.length)
              .find(m => {
                if (pathname === m.href) return true;
                if (m.href === '/admin') return false;
                return pathname?.startsWith(`${m.href}/`);
              });

            const isActive = bestMatch?.name === item.name;

            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center gap-3 px-6 py-3 transition-colors text-[14px] font-medium border-l-4 ${
                  isActive 
                    ? "bg-blue-50/50 text-blue-600 border-blue-600" 
                    : "text-gray-600 border-transparent hover:bg-gray-50 hover:text-slate-900"
                }`}
              >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={isActive ? 2.5 : 2} stroke="currentColor" className="w-[20px] h-[20px]">
      ...
      <div className="p-4 border-t border-gray-200 shrink-0 bg-gray-50/50">
        <div className="flex items-center gap-3 px-2 py-1">
          <div className="w-9 h-9 rounded bg-slate-200 flex items-center justify-center text-slate-600 font-bold shrink-0 text-sm border border-slate-300">
            {role.substring(0, 1)}
          </div>
          <div className="overflow-hidden">
            <p className="text-sm font-bold text-slate-800 truncate">{role.replace('_', ' ')}</p>
